...
//}(this, function (angular, moment) {
    var Module = angular.module('datePickerSchedule', []);

    Module.constant('datePickerConfig', {
        template: 'templates/datePickerSchedule.html',
        view: 'month',
...
//Moment format filter.
    Module.filter('mFormat', function () {
        return function (m, format, tz,isZhcn) {
            if (!(moment.isMoment(m))) {
                return moment(m).format(format);
            }
            var zhanWeekObj={Sun:'日',Mon:'一',Tue:'二',Wed:'三',Thu:'四',Fri:'五',Sat:'六'};
            var zhanMonthObj={January:'一',February:'二',March:'三',April:'四',May:'五',June:'六',July:'七',August:'八',September:'九',October:'十',November:'十一',December:'十二'};
            var zhanSubMonthObj={Jan:'一',Feb:'二',Mar:'三',Apr:'四',May:'五',Jun:'六',Jul:'七',Aug:'八',Sep:'九',Oct:'十',Nov:'十一',Dec:'十二'};
            var result=tz ? moment.tz(m, tz).format(format) : m.format(format);
            if(format=='ddd') return zhanWeekObj[result];
            if(format=='MMM') return zhanSubMonthObj[result]+'月';
            if(format=='YYYY MMMM' && result) return result.split(' ')[0]+'年'+zhanMonthObj[result.split(' ')[1]]+"月";
            return result;
        };
    });
    ...
    Module.directive('datePicker', ['datePickerConfig', 'datePickerUtils', function datePickerDirective(datePickerConfig, datePickerUtils) {
        ...
        return {
            ...
            link: function (scope, element, attrs, ngModel) {
            	scope.isZhcn =scope.$parent.isZhcn;
            	
            	var customClass=scope.$parent.customClass;
                ...
                scope.setView = function (nextView) {
                    ...
                    scope.$root.$broadcast('dateScheduleShow',element.parent())
                };

                scope.$on('scheduleRefresh',function(event,data){
                	scope.childCustomClass = data;
                }); 
                
                scope.getSpecialCalss = function(d){
                	if(scope.childCustomClass && d && scope.childCustomClass.hasOwnProperty(moment(d).format('YYYY-MM-DD'))) return scope.childCustomClass[moment(d).format('YYYY-MM-DD')];
                	return '';
                }
                ...
                setDate = function (date) {
                    ...
                    if (scope.callbackOnSetDate) {
                        scope.callbackOnSetDate(attrs.datePicker, scope.date,scope.weeks);
                    }
                };
                ...
                function update() {
                    ...
                    if (scope.callbackOnSetDate && scope.weeks) {
                        scope.callbackOnSetDate(attrs.datePicker, scope.date ,scope.weeks);
                    }
                    ...
                    if (view === 'date') {
                        ...
                        for (i = 0; i < weeks.length; i++) {
                            ...
                            for (j = 0; j < week.length; j++) {
                            	//scope.childCustomClass
                            	//customClass.hasOwnProperty(moment(d).format('YYYYMMDD')
                                classList = '';
...
//      factory(global.angular, global.moment);
//}(this, function (angular, moment) {
    angular.module('datePickerSchedule').factory('datePickerUtils', function () {
        var tz, firstDay;
...
//}));
    ...
    Module.constant('dateTimeConfig', {
        template: function (attrs, id) {
            return '' +
                '<div  style="box-shadow: none;position: relative;" ' +
                (id ? 'id="' + id + '" ' : '') +
    ...
    Module.directive('dateSchedule', ['$compile', '$document', '$filter', 'dateTimeConfig', '$parse', 'datePickerUtils', function ($compile, $document, $filter, dateTimeConfig, $parse, datePickerUtils) {
        ...
        return {
            ...
            link: function (scope, element, attrs, ngModel) {
                var format = attrs.format || dateTimeConfig.format,
                    ...
                    alwaysShow= attrs.alwaysshow || false,
                ...
                function showPicker() {
                    ...
                    if (position === 'absolute') {
                        ...
                        // relative
                        container = angular.element('<div style="position: relative;" date-picker-wrapper></div>');
                    ...
                    scope.$root.$broadcast('dateScheduleShow',element)
                }
                
                if(!alwaysShow)  element.bind('blur', clear);
...
//}));
    ...
    angular.module('datePickerSchedule').run(['$templateCache', function($templateCache) {
        $templateCache.put('templates/datePickerSchedule.html',
            "<div ng-switch=\"view\">\r" +
            "\n" +
            "  <div ng-switch-when=\"date\">\r" +
            "\n" +
            "    <table style=\"width:{{testWidth}}\">\r" +
            "\n" +
            "      <thead>\r" +
            "\n" +
            "      <tr>\r" +
            ...
            "\n" +
            "        <th colspan=\"5\" ng-if='isZhcn'  class=\"switch\" ng-click=\"setView('month')\" ng-bind=\"date|mFormat:'YYYY MMMM':tz:'zhcn'\"></th>\r" +
            "        <th colspan=\"5\" ng-if='!isZhcn'  class=\"switch\" ng-click=\"setView('month')\" ng-bind=\"date|mFormat:'YYYY MMMM':tz\"></th>\r" +
            "\n" +
            ...
            "      <tr>\r" +
            "\n" +
            "        <th ng-if='isZhcn' ng-repeat=\"day in weekdays\" style=\"overflow: hidden\" ng-bind=\"day|mFormat:'ddd':tz:'zhcn'\"></th>\r" +
            "        <th ng-if='!isZhcn' ng-repeat=\"day in weekdays\" style=\"overflow: hidden\" ng-bind=\"day|mFormat:'ddd':tz\"></th>\r" +
            "\n" +
            "      </tr>\r" +
            "\n" +
            "      </thead>\r" +
            ...
            "\n" +
            "        <td  align=\"center\" ng-repeat=\"day in week\">\r" +
            "\n" +
            "          <span\r" +
            "\n" +
            "            ng-class=\"classes[$index2][$index]\"\r" +
            "\n" +      
            "            ng-click=\"selectDate(day)\" >{{day|mFormat:'DD':tz}}<em class=\"{{getSpecialCalss(day)}}\"></em></span>\r" +
            "\n" +
            "        </td>\r" +
            "\n" +
            "      </tr>\r" +
