...
var ZtreeCreator = function(treeId,url,initJson,dialogService){
	
	if(!treeId) dialogService.alert("构造Ztree必须提供 treeId");
	this.treeId = treeId;
	...
	this.initZtree = function(param,level,callBack){
		if(!url && !_setting.async.url) dialogService.alert("构造Ztree必须提供 请求地址！");
	...
	this.getTreeObj = function(){
		if(!_treeObj) dialogService.alert(treeId+"尚未初始化");
		return _treeObj;
	};
	...
	this.setCallback = function(callBack){
		...
		for(call in callBack){
			if(!jQuery.isFunction(callBack[call])) dialogService.alert(call+" :is not a function");
	 ...
	 * isShowIn,被显示在某个元素下面，比如 input框，做成类似comboTree的样子
