;(function() {
	...
	function calcTableBodyHeight(p){
		...
		if(iboxTarget){
			var targetParent = ibox.closest("." + iboxTarget);
		...
		var titleHeight = ibox.find(".ibox-title").outerHeight() | 0,
			...
			tableBodyHeight = footerHeight - tableHeadHeight - paginationHeight - 10;
		//针对除列表页面用到的table，添加控制高度样式
		ibox.find(".table-body").css("minHeight","300px");
		// 设置表格的高度
	...
	$(function(){
		$("body").ready(function(t){
			...
			$root.$on("dataTable:ready", function(){
				dataTablePerformance($(".layui-layer-content"));
			});
		...
		$(window).bind("resize", function(){
			dataTablePerformance($(".layui-layer-content"), true);
		});
	});
})();