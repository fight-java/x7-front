...
OfficeControl=function(){
	{
		...
		this.config={doctype:'doc',fileId:"",controlId:"officeObj",myNum:'0',right:'w',user:{}};
	};
	/**
	 * 处理文件菜单事件。
	 */
	 ...
	 * 模版套红。
	 */
	this.insertContentTemplate=function(dialogService){
		try{
			this.templatetype = 2;
			...
			if(!(Sys.firefox) && !(Sys.chrome)){      //IE是同步的，它 会等待模版加载成功后执行书签的插入 （火狐和谷歌就不可以）
				...
				if(!_self.controlObj.ActiveDocument.BookMarks.Exists(BookMarkName)){
					dialogService.alert("Word 模板中不存在名称为：\""+BookMarkName+"\"的书签！");
					return false;
				}
...
//		 $("#" +divContainerId +" div[name='menuBar']").ligerMenuBar({ items:items});
	...
	this.saveRemote=function(inputObjNum){
		var path= __ctx + "/platform/system/sysOfficeFile/saveFile.ht";
	 ...
	 * 对文档进行签单
	 */
	this.signature=function(dialogService){
...
//			this.controlObj.AddSignFromURL(retVal.userName,sealUrl);
			...
			dialogService.alert("signature:" +err.name + ": " + err.message);
			return -1;
		}
	};
	...
	this.signaturePdfFromEkey=function(){
		...
		if(this.controlObj!=null){
			...
			this.controlObj.AddPDFSecSignFromEKEY(_self.config.user.name,null,"111111",null,1,null,null,null,null,true,false,true,false,null);
		}			
	};
	 ...
	 * 服务器返回文件id到this.config.fileId，同时返回文件ID。
	 */
	this.officeToPdf=function(dialogService){
		...
		try{
			...
			dialogService.alert("officeToPdf:" +err.name + ": " + err.message);
		}			
	};
...
function saveMethodOnComplete(type, code, html) {
	...
	if(arryNum>=0){
		...
		if(arryValue>0){
			...
			setValToScope(OfficePlugin.fileObjs.get(arryNum),arryValue);
 ...
 * html 为后台返回的内容
 */
function addTemplateOnComplete(myNum,dialogService) {
