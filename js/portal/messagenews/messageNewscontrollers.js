...
//  新闻公告 编辑明细
function messageNewsEditCtrl($rootScope, $scope, $compile, baseService,dialogService,$stateParams, $state,$filter){
	//关闭
	$scope.close = function(){
		 $state.go($stateParams.state);
	}
	$scope.title = $filter('translate')('messageNewsList.view_press_announcement');
	
	$scope.data = {}
	
	baseService
	.get("${portal}/portal/messageNews/v1/get/"+$stateParams.id)
	.then(function(response) {
		$scope.data = response;
	});	
