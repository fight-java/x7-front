...
(function($, window, document, undefined,dialogService) {
	...
	Plugin.prototype = {
		...
		initSetting:function(){
			...
			// 设置布局最小高度
			$(layoutClass).css("min-height", $(window).height() - 150);
			$('.sidebar-nav').css("min-height", $(window).height()-20);
			...
			$.ajax({
				...
				success : function(data) {
					...
					if (obj.result == 1) {
						...
						dialogService.alert({message:obj.message});
		...
		saveRemoteMyLayout:function(){
			...
			this.changeHtml(html);
			this.changeHtml(designHtml);
			var ctr = window.getContext();
			$.ajax({
				type : "POST",
				url : ctr.portal + "/portal/sysIndexMyLayout/v1/saveLayout",
				data : {
					html :  $.base64.encode(html.html(),"utf-8"),
					designHtml:$.base64.encode(designHtml.html(),"utf-8")
				},
				success : function(data) {
					var obj = eval('(' + data + ')');
					if (obj.result == 1) {
						dialogService.confirm("保存成功,是否继续操作？", function(rtn){
							if(rtn){
									window.location.href= "{portal}/portal/sysIndexMyLayout/design";
							}else{
								if(window.opener){
									try{
										window.opener.location.href=window.opener.location.href.getNewUrl();
						...
						dialogService.fail(obj.message);
					}
		...
		deleteLayout:function(id){
			...
			dialogService.confirm("该操作是删除自己的布局，首页的布局取默认的布局。是否删除当前设计的布局？", function(rtn){
				if(rtn){
					
					$.ajax({
						type : "POST",
						url : "{portal}/portal/sysIndexMyLayout/v1/deleteLayout",
						data : {
							id : id
						},
						success : function(data) {
		...
		initOperation:function(){
			...
			//下载（预览）
			$("dt_downloadModal").unbind("click").bind("click",function(e) {
				e.preventDefault();
				me.downloadLayoutSrc();
			});
			$("#button-my-save-modal").unbind("click").bind("click",function(e) {
				e.preventDefault();
			...
			$("#edit").unbind("click").bind("click",function() {
				...
				$("body").addClass("edit");
				$(".index-layout").css("marginLeft","240px");
			...
			$("#clear").unbind("click").bind("click",function(e) {
				e.preventDefault();
				layer.confirm("是否清空布局？", angular.extend({title:"请确认", icon:3},{}), function(index){
					  me.clearIndex();
					  layer.close(index);
				  }, function(index){
					  layer.close(index);
			...
			$(".nav-header-d").unbind("click").bind("click",function() {
	...
	$.fn[pluginName] = function(options) {
		var l = this.each(function(){
			var pluginItem = $.data(this, "plugin_" + pluginName); 
			if(!pluginItem) {
				pluginItem = new Plugin(this, options);
				$.data(this, "plugin_" + pluginName, pluginItem);
			}
			pluginItem.init();
		});
		return 
...
//var scop = $("body").scope();
//scop.$on("$stateChangeSuccess", function(a,b,c,d,e,f){
//	setTimeout(function(){
//		$('#designContainer').indexLayout();
//	}, 500);
//});
